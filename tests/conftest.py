...
Tunes are HVSC copyright works, never committed.  ``tune_path`` fetches a tune
into a gitignored cache (skipping the test when the tune is absent and there is no
network), and ``oracle_grid`` produces the ground-truth per-frame SID register
grid -- live from the ``preframr-sidtrace`` binary (``$SIDTRACE_BIN``) when
available, else from the committed frozen grid -- mirroring deplayroutine's
env-gated validator.
"""
...
def tune_id(request):
    """Parametrize over each Future Composer test tune id."""
